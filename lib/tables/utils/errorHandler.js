        ...
        case 404:
            printErrorMessage(
                `${NOT_FOUND_TEXT} ${action} for repository ${clc.green(
                    repo
                )}, got ${statusCode} code 🚫, please re-check the repo name and reconfigure by running ${clc.green(
                    "okgit switchrepo"
                )}`
            );
            break;
        case 401:
            printErrorMessage(
                `${UNAUTHORIZED_TEXT} to ${action} for repository ${clc.green(
                    repo
                )}, got ${statusCode} code 🚫, please re-check the token and reconfigure okgit by running ${clc.green(
                    "okgit config"
                )}`
            );
            break;
        case 403:
            printErrorMessage(
                `${FORBIDDEN_TEXT} to ${action} for repository ${clc.green(
                    repo
                )}, got ${statusCode} code 🚫, please re-check if you have permissions to access ${repo} or check if yout token has right permissions and then reconfigure okgit by running ${clc.green(
                    "okgit config"
                )}`
            );
            break;
        case 500:
            printErrorMessage(
                `Got ${INTERNAL_SERVER_ERROR_TEXT} while ${action}for repository ${clc.green(
                    repo
                )}, got ${statusCode} code 🚫, please try again later.`
            );
            break;
        case 422:
            printErrorMessage(
