...
export class GithubPullRequest extends BaseAPI {
    ...
    async createPullRequest(title, currentBranchName, baseBranchName) {
        const prData = {
            title: title,
            head: currentBranchName,
            base: baseBranchName,
        };
        const response = await this.postRequest(`/${repo}/pulls`, prData);
    ...
    async showPullRequestComments(id) {
        ...
        const response = await this.getRequest(pullRequestCommentsUrl).catch(
            err => {
                console.error(
                    `Something went wrong while processing the URL${err}`
                );
                return resultsTable;
            }
        );
    ...
    async showPullRequestFiles(id) {
        ...
        const response = await this.getRequest(pullRequestFilesUrl).catch(
            err => {
                console.error(
                    `Something went wrong while processing the request ${err}`
                );
                return resultsTable;
            }
        );
    ...
    async removeReviewers(pullRequestId, reviewerNames) {
        ...
        const response = await this.deleteRequest(
            ...
            console.error(`Got a deadly error ${err}`);
            return resultsTable;
        });
        if (validateSchema(response, pullRequestAddReviewersSchema())) {
            response["requested_reviewers"].forEach(name => {
    ...
    async mergePullRequest(pullRequestId) {
        ...
        const response = await this.putRequest(
            ...
            console.error(`Unable to merge because  ${err} `);
        });
        if (response.status === 200) {
            console.log(`${pullRequestId} ${response.data.message}`);
        } else if (response.status === 405 || response.status === 409) {
            console.log(`Failed to merge because ${response.data.message}`);
        }
    }
