...
export class GithubPullRequest extends BaseAPI {
    ...
    async createPullRequest(title, currentBranchName, baseBranchName, body) {
        ...
        const response = await this.postRequest(`/${repo}/pulls`, prData).catch(
            err => {
                errorHandler(err["status"], "createPullRequest", repo);
            }
        );
        console.log(
            `Created Pull Request ${response.html_url} 🚀 clc.green("use okgit pr <id> --web ") top open in your favorite browser`
        );
    }
